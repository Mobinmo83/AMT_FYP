...
def run_model_on_mel(
    ...
    The demo intentionally does not chunk the input. This preserves the same
    BiLSTM context assumptions as the validation/test evaluation notebook.
    """
...
def gt_rolls_to_note_events(gt: GroundTruthDict) -> list[DemoNoteEvent]:
    ...
    This is the fair metric reference used for validation/test evaluation.
    The original MAESTRO MIDI can still be displayed and downloaded, but the
    metrics in the demo use this cached-roll path to match the report numbers.
    """
    events = rolls_to_note_events(
        onset_roll=gt["onset"],
        frame_roll=gt["frame"],
