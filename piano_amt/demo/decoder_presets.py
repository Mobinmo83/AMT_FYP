...
class AdvancedDecoderConfig:
    """Editable advanced-decoder configuration used by the public demo.

    The field names are deliberately the same names accepted by
    ``models.onsets_frames.decode_advanced.advanced_rolls_to_note_events``.
    This avoids alias filtering and prevents the demo from accidentally running
    different parameters from the final evaluation notebook.
    ...
    def decoder_kwargs(self) -> Dict[str, Any]:
        ...
        Thresholds are passed separately to keep the call identical to
...
# Exact final chosen values from the test-set combination experiments.
# adv_m3_m4:
#   min_note_duration_ms=55.0, use_duplicate_removal=True,
#   duplicate_tolerance_sec=0.06
# adv_m2_m3_m4:
#   use_frame_smoothing=True, frame_smoothing_kernel=3,
...
ADVANCED_DECODER_PRESETS: Dict[str, AdvancedDecoderConfig] = {
    ...
    # Isolated single-method presets are included for live explanation only.
    # These are useful in the demo if an examiner asks what each method does.
...
def make_decoder_config(
    ...
    **kwargs: Any,
    ...
    """Create an editable decoder config from a named preset.

    Example:
        cfg = make_decoder_config(
            "quality_m2_m3_m4",
            min_note_duration_ms=60.0,
            duplicate_tolerance_sec=0.08,
        )
    """
    merged: Dict[str, Any] = {}
    if overrides:
...
def config_table_dict(cfg: AdvancedDecoderConfig) -> Dict[str, Any]:
    """Compact dict for displaying active demo settings in notebooks."""
    return asdict(cfg)
